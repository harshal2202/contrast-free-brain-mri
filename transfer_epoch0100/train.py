...
def load_generator(checkpoint_path, device):
    ...
    if checkpoint_path.lower().endswith((".h5", ".hdf5")):
        with h5py.File(checkpoint_path, "r") as h5_file:
            state = {
                name: torch.as_tensor(h5_file["state_dict"][name][()])
                for name in h5_file["state_dict"]
            }
    else:
